...
체류 시간과 비용은 **조사하지 않고 유형별 규칙으로 정한다.**

조사해도 정확해지지 않기 때문이다. 카페가 "40분 앉으세요"라고 정해주지 않고, 사람마다 다르다.
규칙으로 정하면 값이 일관되고, 왜 그 숫자인지 설명할 수 있다. 설명 가능한 추천이라는 목표에 더 맞는다.

| `placeType` | 체류 시간 | 예상 비용 | 활동 분류 |
| --- | --- | --- | --- |
| 테이크아웃 카페 | 10분 | 4,500원 | 카페 |
| 앉아서 쉬는 카페 | 40분 | 7,000원 | 카페 |
| 베이커리·브런치 | 30분 | 8,000원 | 카페 |
| 서점·소품숍 | 25분 | 2,000원 | 구경 |
| 전시·갤러리 | 40분 | 5,000원 | 문화 |
| 공원·산책로 | 30분 | 0원 | 산책 |

사람이 직접 해야 하는 일은 아래 세 가지뿐이다. 30~60분이면 된다.

1. 카카오 검색 결과에서 추천 후보로 남길 장소 고르기
2. 각 장소를 위 `placeType` 중 하나로 분류하기
3. 규칙표 자체를 조정하기

**손으로 적으면 안 되는 것**: 이동 시간. 출발지마다 다르므로 카카오 경로 API가 계산해야 한다.
**반드시 진짜여야 하는 것**: 장소 이름과 좌표. 없는 장소를 추천하면 서비스가 무너지고, 좌표가 없으면 거리 계산과 지도 표시가 불가능하다.
